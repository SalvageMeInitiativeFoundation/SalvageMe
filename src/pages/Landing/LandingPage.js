...
const LandingPage = (props) => {
  ...
  const MakePayment = async (e) => {
    ...
    const data = {
      ...
      payer: {
        partyIdType: "MSISDN",
        partyId: "+233" + MSISDN.slice(1, MSISDN.length - 1),
      },
      payerMessage: "SalvageME Donation",
    };
    try {
      const paymentResponse = await axios.post(
        `${process.env.REACT_APP_BASE_URL}/salvageme/payment/makePayment`,
        data
      );
  ...
  const verifyPayment = async (e) => {
    ...
    try {
      const paymentResponse = await axios.post(
        `${process.env.REACT_APP_BASE_URL}/salvageme/payment/verifyPayment`,
        data
      );
      //    console.log(loginResponse.data)
      if (paymentResponse.status == 200) {
